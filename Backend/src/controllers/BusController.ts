...
router.get("/", async (req: Request, res: Response) => {
  print("req.query", req.query, req.query.BusStop);
	...
	const num = 20;
	const BusStop = (req.query.BusStop as string) || "Tillerterminalen";
	...
	const response = await fetchWithRetry(`https://api.entur.io/geocoder/v1/search?text=${(BusStop + ", Trondheim").replace(" ", "%20")}`); // autocomplete
	const BussID = await response.json();
	print((BusStop + ", Trondheim").replace(" ", "%20"), BussID, BussID.features[0].properties);
  ...
  print(query);
	...
	try {
		...
		return res.json([BussID, busStop]);
	} catch (err) {
